    ...
    ui(new Ui::startwindow)
{
    ...
    m_service=vehicle_2_coreActivator::getService<Coreservice>("Coreservice");
    vehicle_2_coreActivator::registerservice(this,"vehicle_2_coreservice");

    m_service->addView(UcsDefines::UcsViewKind::VEHICLE_2_WINDOW,this);
    vehicle_2_coreActivator::subscribevent(UcsEventConstants::TOPCI_MENU_CLICKED,this);
}
...
void startwindow::setupwidget()
{
    ...
    addView(UcsDefines::VEHICLE_2_WINDOW,m_whitewidget);
    changeView(UcsDefines::VEHICLE_2_WINDOW);/*根据主页面的值设置按钮值*/
...
void startwindow::EventTriggeres(XTLevent event)
{
    if (event.eventname==UcsEventConstants::TOPCI_MENU_CLICKED)
    {
        ...
        if(name== UcsEventConstants::VEHICLE_2_CONTROL)
        {
            changeView(UcsDefines::VEHICLE2_CONTROL);
            //            changeView(UcsDefines::UCS_VIEW_KIND_MAIN);
            return;
        }
        if(name== UcsEventConstants::VEHICLE_2_MAIN)
        {
            changeView(UcsDefines::VEHICLE_2_WINDOW);
            //            changeView(UcsDefines::UCS_VIEW_KIND_MAIN);
            return;
        }
        if(name== UcsEventConstants::VEHICLE_2_VIDEO)
        {
            changeView(UcsDefines::VEHICLE2_VIDEO);
            //            changeView(UcsDefines::UCS_VIEW_KIND_MAIN);
            return;
        }
    }
...
void startwindow::buildMenu(int type)
{
    ...
    switch (type)
    {
    case UcsDefines::VEHICLE_2_WINDOW:
    {
        setButton(1, QStringLiteral("Control \r\n Mode"), UcsEventConstants::VEHICLE_2_CONTROL, QString(":/b_img/六芒星.png"));
        setButton(2, QStringLiteral("video \r\n display"), UcsEventConstants::VEHICLE_2_VIDEO, QString(":/b_img/六芒星.png"));
        setButton(12, QStringLiteral("EXIT"), UcsEventConstants::EXITTOMAIN, QString(":/b_img/六芒星.png"));
        break;
    }
    case UcsDefines::VEHICLE2_CONTROL:
    {
        setButton(12, QStringLiteral("EXIT"), UcsEventConstants::VEHICLE_2_MAIN, QString(":/b_img/六芒星.png"));
        break;
    }
    case UcsDefines::VEHICLE2_VIDEO:
    {
        setButton(1, QStringLiteral("Control \r\n panel"), UcsEventConstants::VEHICLE_2_CONTROL, QString(":/b_img/六芒星.png"));
...
void startwindow::onMenuClicked()
{
    ...
    vehicle_2_coreActivator::postevent(event);
