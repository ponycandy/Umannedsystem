...
#ifndef XTLEVENTADMIN_H
#define XTLEVENTADMIN_H
 ...
 *
 */
struct xtlEventAdmin
{
  virtual ~xtlEventAdmin() {}
   ...
   *
   */
  virtual void postEvent(const xtlEvent& event) = 0;
   ...
   *
   */
  virtual void sendEvent(const xtlEvent& event) = 0;
   ...
   * following signature:
   * \code
   * someSignal(const xtlDictionary& props)
   * \endcode
   * where <code>props</code> will be used to construct a xtlEvent class which
   ...
   *
   * Slots should be registered with a property xtlEventConstants::EVENT_TOPIC.
   ...
   * \endverbatim
   *
   * Slots can also be registered with a property named xtlEventConstants::EVENT_FILTER.
   ...
   *
   * @throws xtlInvalidArgumentException If <code>subscriber</code> or <code>member</code> is 0
  ...
  virtual qlonglong subscribeSlot(const QObject* subscriber, const char* member,
                                  const xtlDictionary& properties, Qt::ConnectionType type = Qt::AutoConnection) = 0;
   ...
   */
  virtual bool updateProperties(qlonglong subscriptionId, const xtlDictionary& properties) = 0;

};


Q_DECLARE_INTERFACE(xtlEventAdmin, "xyz.xtal.service.event.EventAdmin")

#endif // XTLEVENTADMIN_H
